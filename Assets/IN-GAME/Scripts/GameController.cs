...
namespace FindItFast
{
	...
	public class GameController:MonoBehaviour 
	{
		...
		void CreateLevel()
		{
			...
			if ( edgeLeft && edgeTop && edgeRight && edgeBottom )
			{
				...
				if ( showIconsOnTop == false )
				{
					...
					{
						topBar.Find("Base/FindObjectText").GetComponent<TextMesh>().text = topBar.Find("Base/FindObjectText/Shadow").GetComponent<TextMesh>().text = "FIND " + numberOfHiddenObjects.ToString() + " " + newObject.GetComponent<HiddenObject>().namePlural;
					}
					else
					{
						topBar.Find("Base/FindObjectText").GetComponent<TextMesh>().text = topBar.Find("Base/FindObjectText/Shadow").GetComponent<TextMesh>().text = "FIND " + newObject.GetComponent<HiddenObject>().nameArticle + " " + newObject.GetComponent<HiddenObject>().objectName;
					}
				}
				else
				{
		...
		void UpdateTimer()
		{
			...
			{
				topBar.Find("Base/TimerClock").GetComponent<SpriteRenderer>().color = topBar.Find("Base/TimerClock").GetComponent<AnimateColors>().colorList[0];
